...
def OlexCDS(cell_error_s=2, cell_error_a=2, cell_a='', cell_b='', cell_c='', cell_alpha='', cell_beta='', cell_gamma=''):
  ...
  connection.request(
    "POST", "/cgi-bin/login/loginhandler.cgi", login_params, head)
  ...
  if response.status == 200:
    ...
    print("Using errors of angles: ", cell_error_a,
        " and lengths: ", cell_error_s)
    ...
    print("Using following cell angles: ",
    ...
    head = {'Cookie': DLCOOKIE, "action": "cw10",
    ...
    if response.status == 200:
      ...
      for line_response in response.read().splitlines():
        ...
        if "hits for your search" in line_response:
          ...
          cds_hits_string = re.search(
            r'(?<=name\=\"hitfile\")*value\=\"/cweb/.*?\"', results_line)
          # print cds_hits_string
          # print cds_hits_string.group()
          cds_hitfile = cds_hits_string.group().split("\"")[1]
          # print cds_hitfile
          number_of_hits = re.search(
            r'\d+', strip_html(results_line))
          number_of_hits_found = int(number_of_hits.group())
          if number_of_hits_found == 0:
            print("No results found\n")
            print(
              "Try increasing the error in your search parameters e.g. spy.olexcds(3,3)")
          elif number_of_hits_found > 0:
            print(
              "There are %d hits found do you wish to view them via CrystalWeb?" % number_of_hits_found)
            # print "There are less than 10 hits we are getting the hits now"
            head = {'Cookie': DLCOOKIE, "action": "cwd4",
                "enctype": "application/x-www-form-urlencoded"}
            ...
            connection.request(
              "POST", "/cgi-bin/cweb/cwd4", result_params, head)
            response = connection.getresponse()
            ...
            CDS_res = open('%s/%s_CDS.html' %
                     (OV.FilePath(), OV.FileName()), 'w')
            #CDS_res = open('test_CDS.html', 'w')
            CDS_res.write("%s" % search_results_out)
            CDS_res.close()
            olx.Shell('%s/%s_CDS.html' %
          ...
          if number_of_hits_found > 0:
            print(
              "There are %d hits, < 10 hits will be displayed > 10 openned in a browser" % number_of_hits_found)
            head = {'Cookie': DLCOOKIE, "action": "cwd4",
                "enctype": "application/x-www-form-urlencoded"}
            result_params = urlencode({
              "Display hits": "Display hits",
            ...
            for line in lines:
              ...
              if CDS_hit_yes_no > 0:
                ...
                if "LengthsAngles" in lines[i]:
                  fishing = re.search(
                    'a(?P<a>\d+.\d+)alpha(?P<alpha>\d+)', lines[i+1])
                  compound_cell_a = float(fishing.group('a'))
                  compound_cell_alpha = float(
                    fishing.group('alpha'))
                  fishing = re.search(
                    'b(?P<b>\d+.\d+)beta(?P<beta>\d+)', lines[i+2])
                  compound_cell_b = float(fishing.group('b'))
                  compound_cell_beta = float(
                    fishing.group('beta'))
                  fishing = re.search(
                    'c(?P<c>\d+.\d+)gamma(?P<gamma>\d+)', lines[i+3])
                  compound_cell_c = float(fishing.group('c'))
                  compound_cell_gamma = float(
                    fishing.group('gamma'))
  ...
  %s|%s|%s|%s|%s
        """ % (CDS_hit_no,
                ...
                # compound_formula,
                CDS_hit_result[CDS_hit_no][3],
                CDS_hit_result[CDS_hit_no][2],  # compound_name
                # compound_reference,
                CDS_hit_result[CDS_hit_no][4],
                # compound_cell_a,
                CDS_hit_result[CDS_hit_no][5],
                # compound_cell_b,
                CDS_hit_result[CDS_hit_no][6],
                # compound_cell_c,
                CDS_hit_result[CDS_hit_no][7],
                # compound_cell_alpha,
                CDS_hit_result[CDS_hit_no][8],
                # compound_cell_beta,
                CDS_hit_result[CDS_hit_no][9],
                # compound_cell_gamma,
                CDS_hit_result[CDS_hit_no][10],
                # compound_cell_volume,
                CDS_hit_result[CDS_hit_no][16],
                # compound_system,,
                CDS_hit_result[CDS_hit_no][11],
                # compound_spgr,,
                CDS_hit_result[CDS_hit_no][12],
                # compound_spgr_no,,
                CDS_hit_result[CDS_hit_no][13],
                # compound_Rf,,
                CDS_hit_result[CDS_hit_no][14],
